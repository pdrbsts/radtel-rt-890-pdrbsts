...
static void DrawMode0(void)
{
	...
	if (gSettings.DtmfState == DTMF_STATE_STUNNED) {
		...
		UI_DrawStatusIcon(4, ICON_LOCK, gSettings.Lock, COLOR_BLACK);
	}
	...
	UI_DrawRoger();
	UI_DrawRepeaterMode();
	UI_DrawStatusIcon(139, ICON_BATTERY, true, COLOR_BLACK);
	UI_DrawBattery();
}
...
void UI_DrawMain(uint8_t Mode)
{
	...
	case 0:
		...
		break;
	case 1:
		DISPLAY_Fill(0, 159, 0, 81, COLOR_WHITE);
		DISPLAY_DrawRectangle0(0, 41, 160, 1, gSettings.BorderColor);
		break;
	}
...
void UI_DrawRepeaterMode(void)
{
	...
	case 1:
		UI_DrawStatusIcon(109, ICON_RR, true, COLOR_BLACK);
		break;
	case 2:
		UI_DrawStatusIcon(109, ICON_TR, true, COLOR_BLACK);
		break;
	default:
		UI_DrawStatusIcon(109, ICON_TR, false, COLOR_BLACK);
		break;
	}
}
